...
suite("cloud/cloudController", () => {
  ...
  suite("showMenu", () => {
    ...
    test("shows link/deploy options when logged in but no app", async () => {
      ...
      assert.ok(items.some((i: any) => i.id === "deploy"))
    ...
    test("calls runDeploy when deploy selected and no app", async () => {
      const deps = createController()
      const workspaceRoot = vscode.Uri.file("/tmp/test")
  ...
  suite("refresh", () => {
    test("shows sign in text when not logged in", async () => {
      ...
      assert.strictEqual(
        deps.statusBar.text,
        "$(cloud) Sign in to FastAPI Cloud",
      )
  ...
  suite("runDeploy", () => {
    ...
    test("runs deploy when workspace root is set", async () => {
      const deps = createController()
      await initializeWithApp(deps)
